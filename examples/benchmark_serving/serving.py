...
from structs import TestRequest, Dataset, RequestResult, slo_percentile
from dataset import get_test_requests

from partitioning.utils import profile_power
...
def main(args: argparse.Namespace):
    ...
    throughput_req = args.num_prompts / benchmark_time
    throughput_token = sum([req.prompt_len + req.output_len for req in input_requests]) / benchmark_time
    print(f"Total time: {benchmark_time:.2f} s")
    print(f"Throughput:")
    print(f"\t{throughput_req:.2f} requests/s")
    print(f"\t{throughput_token:.2f} tokens/s")
    ...
    node_powers = profile_power(
        devices=devices,
        duration=benchmark_time,
        step=1
    )
    sum_power = sum(node_powers.values())
    print(f"\t{devices}")
    print(f"\t{[node_powers[d] for d in devices]} W")
    print(f"Sum Avg Power: {sum_power:.2f} W")

    print(f"Energy Efficiency:")
    print(f"\t{throughput_req / sum_power:.2f} requests/J")
    print(f"\t{throughput_token / sum_power:.2f} tokens/J")

    p50_ttft, p50_tpot = slo_percentile(request_results, 50)
    print(f"P50 TTFT & TPOT:\n\t{p50_ttft*1000:.2f}ms, {p50_tpot*1000:.2f}ms")
    p90_ttft, p90_tpot = slo_percentile(request_results, 90)
    print(f"P90 TTFT & TPOT:\n\t{p90_ttft*1000:.2f}ms, {p90_tpot*1000:.2f}ms")
    p95_ttft, p95_tpot = slo_percentile(request_results, 95)
    print(f"P95 TTFT & TPOT:\n\t{p95_ttft*1000:.2f}ms, {p95_tpot*1000:.2f}ms")

    if args.exp_result_dir:
        args.output = os.path.join(args.exp_result_dir, f"{args.num_prompts}-{args.request_rate}.exp")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--backend", type=str, default="adaptsplit", choices=["adaptsplit", "distserve"]
    )
    parser.add_argument("--host", type=str, default="localhost")
    parser.add_argument("--port", type=int, default=32313)
    parser.add_argument(
    ...
    python serving.py \
        ...
        --host localhost \
        --port 32313 \
        --dataset /home/austin/datasets/test_request_datasets/sharegpt-opt-1.3b.bin \
        --num-prompts 64 \
        --request-rate 3 \
        --exp-result-dir /home/austin/repos/AdaptSplit/evals/stats/serving/ \
        --verbose
    '''
