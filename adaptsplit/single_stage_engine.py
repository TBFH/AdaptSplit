...
from adaptsplit.worker import ParaWorker
from adaptsplit.prefill_stage_scheduler import PrefillStageSchedConfig, PrefillStageScheduler, get_prefill_stage_scheduler
from adaptsplit.decoding_stage_scheduler import DecodingStageSchedConfig, DecodingStageScheduler, get_decoding_stage_scheduler
...
class SingleStageLLMEngine(ABC):
    """
    SingleStageLLMEngine: An LLMEngine that runs either the prefill stage or the decoding stage.
    
    This class is the base class for PrefillStageLLMEngine and DecodingStageLLMEngine.
    """
    @abstractmethod
    def _get_scheduler(self) -> PrefillStageScheduler | DecodingStageScheduler:
    ...
    async def initialize(self):
        ...
        self.scheduler: PrefillStageScheduler | DecodingStageScheduler = self._get_scheduler()
...
class PrefillStageLLMEngine(SingleStageLLMEngine):
    def _get_scheduler(self) -> PrefillStageScheduler:
...
class DecodingStageLLMEngine(SingleStageLLMEngine):
    ...
    def init_migrate_pairs(self):
        ...
        def check_overlap(prefill: ParallelConfig, decode: ParallelConfig):
            ...
            if overlap:
                ...
                return (prefill_idxes[0], prefill_idxes[-1] + 1), (decode_idxes[0], decode_idxes[-1] + 1)
            else:
                return None
    ...
    async def _migrate_blocks(
        ...
        """
        Migrate one request from the prefill engine to the decoding engine
