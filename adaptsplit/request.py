...
class MigratingRequest:
    ...
    Each MigratingRequest represents a request that:
      - Has finished the prefill stage
      - Has not yet acceptted by the decoding stage
      - Its block is still on prefill stage's GPU memory (i.e. migration needed)
      
    Those requests are produced by PrefillStageLLMEngine, queued in the "bridge"
